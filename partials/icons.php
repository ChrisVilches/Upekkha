...
<div class="flex space-x-2 justify-center">
  <? foreach (CustomTheme::$social_sites as $site => $data) : ?>
    ...
    <? if ($site == "line") : ?>
      <div x-data="lineQr('<?= $value ?>')">
        <button aria-label="Line" @click="loadQr" data-modal-target="<?= $prefix .
            $site ?>-modal" data-modal-toggle="<?= $prefix .
    $site ?>-modal" type="button" data-tooltip-target="<?= $prefix .
    $site ?>-tooltip" class="<?= $data[
    "class"
] ?> size-10 flex items-center border-black justify-center bg-transparent text-slate-400 rounded-md transition-colors duration-500">
    ...
    <? else : ?>
      <a aria-label="<?= $data["label"] ?>" data-tooltip-target="<?= $prefix .
    $site ?>-tooltip" href="<?= $value ?>" target="_blank" class="<?= $data[
    "class"
] ?> size-10 flex items-center border-black justify-center bg-transparent text-slate-400 rounded-md transition-colors duration-500">
        <i class="fa-<?= $data["fa-icon-type"] ?> fa-<?= $data["fa-icon"] ?>"></i>
      </a>
    <? endif ?>

    <div id="<?= $prefix .
        $site ?>-tooltip" role="tooltip" class="absolute z-10 invisible inline-block px-3 py-2 text-sm font-medium text-white transition-opacity duration-300 bg-gray-900 rounded-lg shadow-sm opacity-0 tooltip">
      <?= $data["label"] ?>
